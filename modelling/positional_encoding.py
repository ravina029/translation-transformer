...
class PositionalEncoding(nn.Module):
    ...
    def forward(self, x: Tensor) -> Tensor:
        if x.ndim != 3:
            raise ValueError( "x must have shape " "(batch_size, sequence_length, hidden_size)"   )
        ...
        if sequence_length > self.max_length:
            raise ValueError( f"sequence length {sequence_length} exceeds " f"max_length={self.max_length}" )
