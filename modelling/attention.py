...
class Attention(nn.Module):
    ...
    def forward(self, query: Tensor, key: Tensor, value: Tensor, attention_mask: Tensor,)-> Tensor:
        # Calculate QK^T
        ...
        if self.mask_future:
            ...
            future_mask=torch.triu(torch.ones(query_length, key_length, dtype=torch.bool, device=scores.device),diagonal=1,)
...
class MultiHeadAttention(nn.Module):
    ...
    def __init__(self, 
        ...
        if hidden_size<=0:
            raise ValueError("hidden size must be positive," f"got {hidden_size}")

        if number_of_heads<=0:
            raise ValueError("number of heads must be positive," f"got {number_of_heads}")

        if hidden_size%number_of_heads !=0:
            raise ValueError(f"Hidden size ({hidden_size}) must be divisible by," f"number of heads ({number_of_heads})")
    ...
    def _split_heads(self, tensor:Tensor) -> Tensor:
        ...
        if hidden_size != self.hidden_size:
            raise ValueError(f"Expected tensor hidden size {self.hidden_size}, but got {hidden_size}")

        tensor = tensor.reshape(batch_size, sequence_length, self.number_of_heads,self.head_size)
    ...
    def _merge_heads(self,tensor:Tensor) -> Tensor:
        ...
        if number_of_heads != self.number_of_heads:
            raise ValueError(f"Expected number of heads are {self.number_of_heads}, but got {number_of_heads}")

        if head_size != self.head_size:
            raise ValueError(f"Expected head size is {self.head_size}, but got {head_size}")
        
        #  apply (1,2) transpose on (B, H, L, Dh) -> (B, L, H, Dh)
        tensor =tensor.transpose(1,2).contiguous()
        # reshape to (B, L, hidden_size)
        return tensor.reshape(batch_size,sequence_length, self.hidden_size, )
    ...
    def forward(
        ...
        if query.ndim !=3:
                    raise ValueError("query must have shape " "(batch_size, query_length, hidden_size)")

        if key.ndim !=3:
                    raise ValueError("key must have shape " "(batch_size, key_length, hidden_size)")

        if value.ndim !=3:
                    raise ValueError("value must have shape " "(batch_size, key_length, hidden_size)")
        ...
        if query.size(-1) != self.hidden_size or key.size(-1) != self.hidden_size or value.size(-1) != self.hidden_size:
                    raise ValueError("The final dimension of query, key, and value must equal "
        ...
        if attention_mask is not None:
            if attention_mask.shape != (batch_size, key_length):
                  raise ValueError(f"attention mask must have shape ({batch_size}, {key_length}),"
                                   f"but received {tuple(attention_mask.shape)}"
                                   )
        ...
        if self.mask_future:
             future_mask = torch.triu(
                  torch.ones(query_length, 
                             key_length, dtype=torch.bool, 
        ...
        head_outputs = torch.matmul(attention_weights,projected_value, )
